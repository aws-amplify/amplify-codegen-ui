...
export const useDataStoreCollection = <M extends PersistentModel>({
  ...
  const fetch = () => {
    ...
    const subscription = DataStore.observeQuery(model, criteria, pagination).subscribe(
...
export function useDataStoreBinding<Model extends PersistentModel>(
  props: DataStoreBindingProps<Model, 'record'> | DataStoreBindingProps<Model, 'collection'>,
): DataStoreItemResult<Model> | DataStoreCollectionResult<Model> {
  return props.type === 'record' ? useDataStoreItem(props) : useDataStoreCollection(props);
}
